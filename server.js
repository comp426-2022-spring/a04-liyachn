import { logdb } from './database.js' //import from database.js
import morgan from 'morgan'
import fs from 'fs'
import express from 'express' //call dependency - ES module
const app = express()
app.use(express.json()) // pass message as json
app.use(express.urlencoded({extended: true})) // pass message as url

import minimist from 'minimist' // Require minimist module (make sure you install this one via npm).
...
if (args.debug == true)
{
    app.get('/app/log/access/', (req, res) => {
        app.get('/app/log/access', (req, res) => {
            const stmt = db.prepare('SELECT * FROM accesslog').all() // log all data in accesslog
